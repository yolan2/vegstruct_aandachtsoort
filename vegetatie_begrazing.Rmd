...
veg_path       <- "C:/Users/yolan/OneDrive/Documenten/UGENT/Master/Stage/vegetatiekaart.gpkg"
grazing_folder <- "C:/Users/yolan/OneDrive/Documenten/UGENT/Master/Stage/hoogtemetrieken"

# --- 1. load vector ---
veg_vect <- vect(veg_path)

# --- 2. load grazing raster as template (no extra “)”) ---
template <- rast("C:/Users/yolan/OneDrive/Documenten/UGENT/Master/Stage/hoogtemetrieken/grazing_density.tif")
...
for (cls in classes) {
  ...
  # 3a) rasterize with fun="count" → counts of polygons per cell
  r_count <- rasterize(
    x          = sub_vect,
    y          = template,
    fun        = "count",
...
cls_files   <- list.files(veg_ras_folder, pattern="\\.tif$", full.names=TRUE)
...
g_files <- c(
  maa     = file.path(grazing_folder, "grazing_diff_norm_month_03.tif"),
  ...
  sep     = file.path(grazing_folder, "grazing_diff_norm_month_09.tif"),
  density = file.path(grazing_folder, "grazing_density.tif")
)
