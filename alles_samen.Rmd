    ...
    if (!is.null(bm) && !is.null(pm)) {
      ...
      mr$combined_cox <- inner_join(fb, fp, by = "Predictor") %>%
        transmute(
          ...
          comb_cox_pval = 2 * (1 - pnorm(abs((bin_mean + pos_mean) / sqrt(bin_sd^2 + pos_sd^2))))
        )
    }

    # combinatie: bin + gamma
    if (!is.null(bm) && !is.null(gm)) {
      ...
      mr$combined_gamma <- inner_join(fb, fg, by = "Predictor") %>%
        transmute(
          ...
          comb_gam_pval = 2 * (1 - pnorm(abs((bin_mean + gam_mean) / sqrt(bin_sd^2 + gam_sd^2))))
        )
    }
...
full_summary <- map_dfr(species_names, function(sp) {
  ...
  imap_dfr(res, function(mr, mn) {
    # binaire model
    bin <- if (!is.null(mr$binary_model)) {
      as.data.frame(mr$binary_model$summary.fixed) %>%
        rownames_to_column("Predictor") %>%
        rename(bin_mean = mean, bin_sd = sd, bin_lower90 = `0.025quant`, bin_upper90 = `0.975quant`) %>%
        mutate(
          bin_pval = 2 * (1 - pnorm(abs(bin_mean / bin_sd))),
          Species = sp,
          Metric = mn
        ) %>%
        select(Species, Metric, Predictor, bin_mean, bin_sd, bin_pval, bin_lower90, bin_upper90)
    } else NULL

    # positieve (box-cox) model
    pos <- if (!is.null(mr$positive_model$model)) {
      as.data.frame(mr$positive_model$model$summary.fixed) %>%
        rownames_to_column("Predictor") %>%
        rename(pos_mean = mean, pos_sd = sd, pos_lower90 = `0.025quant`, pos_upper90 = `0.975quant`) %>%
        mutate(
          pos_pval = 2 * (1 - pnorm(abs(pos_mean / pos_sd))),
          Species = sp,
          Metric = mn
        ) %>%
        select(Species, Metric, Predictor, pos_mean, pos_sd, pos_pval, pos_lower90, pos_upper90)
    } else NULL

    # positieve gamma model(alternatief)
    gam <- if (!is.null(mr$gamma_model)) {
      as.data.frame(mr$gamma_model$summary.fixed) %>%
        rownames_to_column("Predictor") %>%
        rename(gam_mean = mean, gam_sd = sd, gam_lower90 = `0.025quant`, gam_upper90 = `0.975quant`) %>%
        mutate(
          gam_pval = 2 * (1 - pnorm(abs(gam_mean / gam_sd))),
          Species = sp,
          Metric = mn
        ) %>%
        select(Species, Metric, Predictor, gam_mean, gam_sd, gam_pval, gam_lower90, gam_upper90)
    } else NULL

    # gecombineerd bin + box-cox
    cox <- if (!is.null(mr$combined_cox)) {
      mr$combined_cox %>%
        mutate(Species = sp, Metric = mn) %>%
        select(Species, Metric, Predictor, comb_cox_mean, comb_cox_sd, comb_cox_pval)
    } else NULL

    # gecombineerd bin + gamma
    cg <- if (!is.null(mr$combined_gamma)) {
      mr$combined_gamma %>%
        mutate(Species = sp, Metric = mn) %>%
        select(Species, Metric, Predictor, comb_gam_mean, comb_gam_sd, comb_gam_pval)
    } else NULL

    # alles samenvoegen
    list(bin, pos, gam, cox, cg) %>%
...
if (nrow(full_summary) > 0) {
  kable(full_summary, caption = "Binary, Positive, Gamma & Combined Effects per Predictor")
} else {
