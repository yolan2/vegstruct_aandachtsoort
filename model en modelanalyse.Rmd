...
analyze_species <- function(path, chm_metrics) {
  ...
  d0      <- distance(bin_ras)
  ras[d0 >= 400 & ras == 0] <- NA #we zijn niet heel veel met extreem veel nuldata. Dat zorgt enkel voor extra data voor het ruimtelijke model. Daarom beperken we de nulwaarnemingen tot 40m van de dichtsbijzijnde record die niet nul is.
  ...
  for (m in names(chm_metrics)) {
    message("  Metric: ", m)
    print(paste("Class of m:", class(m)))
    if (m == "begrazing"){
      ras_bin <- aggregate(ras_bin, fact = 5, fun = sum, na.rm = TRUE)
      ras     <- aggregate(ras,     fact = 5, fun = sum, na.rm = TRUE)
      ras_bin <- ifel(ras_bin > 0, 1, 0)
    ...
    # Spatial structure
    # Prepare spatial coordinates and neighborhood structure
    coords <- as.matrix(df[, .(x, y)])
    
    # Skip species with too few data points for spatial modeling
    if (nrow(coords) < 30 || length(unique(df$species_binary)) < 2) {
      warning("Skipping due to too few points or no binary variation")
      next
    }
    ...
    # Check if neighborhood has isolated points
    if (any(card(nb) == 0)) {
      warning("Some points have no neighbors — skipping this metric")
      next
    }
    
    # Create adjacency matrix and INLA graph
    W <- as(nb2mat(nb, style = "B"), "sparseMatrix")
    diag(W) <- 0  # Ensure no self-loops
    
    if (nrow(W) != nrow(df)) {
      warning("Mismatch between W and df rows — skipping")
      next
    }
    ...
    print(summary(df$chm))
    print(table(df$species_binary))
    stopifnot(!anyNA(df$chm), !any(is.infinite(df$chm)))
    stopifnot(length(unique(df$species_binary)) > 1)
    ...
    mp  <- NULL
    if (nrow(dfp) > 0) {
      coords_pos <- as.matrix(dfp[, .(x, y)])
      nb_pos     <- knearneigh(coords_pos, k = 8) |> knn2nb(sym = TRUE)
      W_pos      <- as(nb2mat(nb_pos, style = "B"), "sparseMatrix"); diag(W_pos) <- 0
      g_pos      <- inla.read.graph(W_pos)
      dfp[, spatial_id := .I]  # Reindex to match g_pos
    
      mp <- inla(
        species ~ chm + f(spatial_id, model = "besag", graph = g_pos),
