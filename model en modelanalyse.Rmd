...
analyze_species <- function(path, chm_metrics) {
  ...
  for (m in names(chm_metrics)) {
    ...
    if (nrow(df) == 0 || sum(df$species_binary) == 0) next
        ...
        # Check for zero-only CHM values or insufficient overlap

    if (sum(df$species > 0.1 & df$chm > 0) < 30) {
      warning("Less than 30 overlapping values between species and CHM — skipping species.")
      next
    }

    message("Calculating neighborhood structure")

    # Adaptive Neighborhood Calculation
    ...
    if (k > 16) {
      warning("Failed to create connected graph — skipping this metric")
      next
    }
    ...
    if (nrow(dfp) > 0) {
      ...
      repeat {
        ...
        nb_pos <- knearneigh(coords_pos, k = k_pos) |> knn2nb(sym = TRUE)
        if (all(card(nb_pos) > 0) || k_pos > 16) break
        k_pos <- k_pos + 4
      }

      if (k_pos <= 16) {
        W_pos <- as(nb2mat(nb_pos, style = "B"), "sparseMatrix")
        diag(W_pos) <- 0
